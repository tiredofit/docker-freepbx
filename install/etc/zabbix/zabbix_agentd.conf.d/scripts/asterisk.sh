...
function status() {
  proc_status=$(pgrep -f 'sudo -u asterisk /usr/sbin/asterisk')
...
function sip.peers() {
  ...
  if [ -n "$TRUNK" ]; then
    {
      sudo -u asterisk /usr/sbin/asterisk -rx "module unload chan_sip.so"
      sudo -u asterisk /usr/sbin/asterisk -rx "module unload chan_pjsip.so"
      sudo -u asterisk /usr/sbin/asterisk -rx "module load chan_sip.so"
      sudo -u asterisk /usr/sbin/asterisk -rx "module load chan_pjsip.so"
    } >> /dev/null
    sleep 1
    echo "$TRUNK"
  else
